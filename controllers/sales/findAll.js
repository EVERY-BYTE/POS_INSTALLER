...
const findAllSales = async (req, res) => {
    const { error, value } = (0, validateRequest_1.validateRequest)(saleSchema_1.findAllSalesSchema, req.query);
    if (error) {
    ...
    try {
        const { page: queryPage, size: querySize, search, pagination, salesCategory, startDate, endDate } = value;
